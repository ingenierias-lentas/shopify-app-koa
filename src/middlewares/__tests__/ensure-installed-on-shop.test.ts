...
describe('ensureInstalledOnShop', () => {
  ...
  beforeEach(async () => {
    ...
    // Add the ensureInstalledOnShop middleware
    router.use('/test/(.*)', shopify.ensureInstalledOnShop());

    // Define the /test/shop route
  ...
  it('returns 400 if no shop provided', async () => {
    await request(app.callback()).get(`/test/shop`).expect(400);
  });

  it('returns 422 if invalid shop provided', async () => {
    await request(app.callback()).get(`/test/shop?shop=invalid-shop`).expect(422);
  });
  ...
  it('returns 400 if no host provided', async () => {
    ...
    await request(app.callback()).get(`/test/shop?shop=${TEST_SHOP}`).expect(400);
  });
  ...
  it('does NOT redirect to auth if shop NOT installed AND url is exit iFrame path', async () => {
    const encodedHost = Buffer.from(SHOPIFY_HOST, 'utf-8').toString('base64');
    router.use('/exitiframe', shopify.ensureInstalledOnShop());
    router.get('/exitiframe', async (ctx) => {
      ctx.body = 'exit iFrame';
    });
