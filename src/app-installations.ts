...
export class AppInstallations {
  ...
  constructor(config: AppConfigInterface) {
    if (!config.sessionStorage.findSessionsByShop) {
      throw new Error(
        'To use this Koa package, you must provide a session storage manager that implements findSessionsByShop',
      );
    }
    if (!config.sessionStorage.deleteSessions) {
      throw new Error(
        'To use this Koa package, you must provide a session storage manager that implements deleteSessions',
      );
    }
