...
import {SHOPIFY_KOA_LIBRARY_VERSION} from './version';
...
function apiConfigWithDefaults<Params extends Partial<ApiConfigParams>>(
  ...
  let userAgent = `Shopify Koa Library v${SHOPIFY_KOA_LIBRARY_VERSION}`;

  if (apiConfig.userAgentPrefix) {
    userAgent = `${apiConfig.userAgentPrefix} | ${userAgent}`;
  }

  /* eslint-disable no-process-env, @typescript-eslint/no-non-null-asserted-optional-chain */
  return {
  ...
  /* eslint-enable no-process-env, @typescript-eslint/no-non-null-asserted-optional-chain */
}
...
function deprecated(warningFunction: Shopify['logger']['warning']) {
  return function (version: string, message: string): Promise<void> {
    if (semver.gte(SHOPIFY_KOA_LIBRARY_VERSION, version)) {
